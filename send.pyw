...
class FluxApp:
    ...
    def cleanup(self):
        ...
        self.close_flux_instances()

    def close_flux_instances(self):
        if sys.platform == "win32":
            os.system("taskkill /F /IM flux.exe 2>NUL")
        else:
            os.system("pkill -9 flux 2>/dev/null")
    ...
    async def check_flux_installed(self):
        """Check if flux is installed on the system."""
        loading_dialog = ft.AlertDialog(
            modal=True,
        ...
        try:
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    await self.install_flux()
                    return False

            except asyncio.TimeoutError:
                # If the process times out, assume flux is not installed
                loading_dialog.open = False
                self.page.update()
                await self.install_flux()
                return False
            ...
            error_dialog = ft.AlertDialog(
                ...
                content=ft.Column(
                    controls=[
                        ...
                        ft.Container(height=10),
                        ft.Text(
                            f"An error occurred while checking flux installation:\n{str(e)}",
                            size=16,
                            text_align=ft.TextAlign.CENTER,
                        ),
                    ],
    ...
    #             if self.check_flux_installed():
    #                 success_dialog = ft.AlertDialog(
    #                     modal=True,
    #                     title=ft.Text("Success"),
    #                     content=ft.Text("flux has been successfully installed!"),
    #                     actions=[
    #                         ft.TextButton(
    ...
    #                 content=ft.Text(
    #                     f"Failed to install flux: {str(e)}\n"
    #                     "Please install it manually:\n"
    #                     "Windows: winget install schollz.flux\n"
    ...
    #         modal=True,
    #         title=ft.Text("flux Not Found"),
    #         content=ft.Text(
    #             "flux is not installed on your system. Would you like to install it now?"
    #         ),
    #         actions=[
    #             ft.TextButton(
    #                 "Yes",
    ...
    def get_sudo_password(self):
        ...
        password_dialog = ft.AlertDialog(
            ...
            content=ft.Column(
                [
                    ft.Text("Please enter your sudo password to install flux:"),
                    password_input,
                ]
            ),
            actions=[
    ...
    def run_flux_command(self, command):
        try:
            ...
            for line in iter(self.current_process.stdout.readline, ""):
                ...
                if line.strip():
                    ...
                    if "Code is:" in line:
                        ...
                        if code_match:
                            self.flux_code = code_match.group(1)
    ...
    def update_code_display(self):
        if self.code_text and self.flux_code:
            self.code_text.value = f"Code: {self.flux_code}"
            ...
            self.code_note.value = (
                ...
                f"Windows: flux {self.flux_code}\n"
                f'Linux/MacOS: flux_SECRET="{self.flux_code}" flux'
            )
    ...
    def start_flux_command(self, e):
        if not self.process_running:
            ...
            command = f"flux send {file_paths}"

            threading.Thread(
                target=self.run_flux_command, args=(command,), daemon=True
            ).start()
        else:
            self.page.open(
                ft.SnackBar(
    ...
    def check_flux_running(self):
        if sys.platform == "win32":
            cmd = 'tasklist /FI "IMAGENAME eq flux.exe" /NH'
            output = subprocess.check_output(cmd, shell=True).decode()
            return "flux.exe" in output
        else:
            try:
                cmd = "pgrep flux"
    ...
    def show_flux_running_dialog(self):
        def close_flux():
            if sys.platform == "win32":
                os.system("taskkill /F /IM flux.exe 2>NUL")
            else:
        ...
        def show_warning():
            ...
            warning_dialog = ft.AlertDialog(
                ...
                content=ft.Text(
                    "Multiple flux sessions running in background may impact system performance."
                ),
                actions=[
                    ft.TextButton(
                        "OK",
        ...
        dialog = ft.AlertDialog(
            modal=True,
            title=ft.Text("flux Process Running"),
            content=ft.Text(
                "A flux process is already running in the background. "
                "This might interfere with the application. "
                "Would you like to close it?"
            ),
            actions=[
                ft.TextButton("Yes", on_click=lambda _: close_flux()),
    ...
    async def main(self, page: ft.Page):
        ...
        page.window.icon = "./assets/fluxodile.svg"
        ...
        if not await self.check_flux_installed():
            self.install_flux()
        else:
            # Continue with the rest of your initialization
            if self.check_flux_running():
                self.show_flux_running_dialog()
        ...
        title_row = ft.Row(
            [
                ft.Image(
                    src="./assets/fluxodile.svg",
                    width=30,
                    height=30,
                    color=ft.colors.ON_SURFACE,
                ),
                ft.Text("flux GUI", size=24, weight=ft.FontWeight.BOLD),
                ft.IconButton(
        ...
        self.send_button = ft.ElevatedButton(
            ...
            on_click=self.start_flux_command,
            style=ft.ButtonStyle(
